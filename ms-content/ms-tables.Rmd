...
make_data_table_one = function(dat, keep_sex, keep_life_stage) {
  ...
  p_success = rbind(
    with(dat_sub, tapply(!y_is_zero, list(year, origin), function(x) {y = mean(x); msdown::percentize(y)})),
    "All Years" = with(dat_sub, tapply(!y_is_zero, origin, function(x) {y = mean(x); msdown::percentize(y)}))
  )
