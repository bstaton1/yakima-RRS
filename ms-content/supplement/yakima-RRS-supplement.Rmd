...
author: |
    ...
    | <small>**Date Drafted**: 2024-07-26</small>
output:
  html_document
params:
  output_subdir: 
...
protect = c("protect", "fig_id", "tab_id", "the_fig", "the_tab", "proj_dir", "supp_dir", "the_args")

# initiate the id counters for figure and table IDs
fig_id <<- 0; tab_id <<- 0

# create functions to print/increment these counters
# inc = TRUE will increment the counter +1 before printing
the_fig = function(inc = TRUE) {fig_id <<- fig_id + ifelse(inc, 1, 0); fig_id}
the_tab = function(inc = TRUE) {tab_id <<- tab_id + ifelse(inc, 1, 0); tab_id}
...
no_F1_broodstock_footnote = "Removed because spawners (F~0~) with progeny (F~1~) used as broodstock will have biased grand-progeny (F~2~) counts, due to improved juvenile survival of HOR fish relative to NOR fish. Thus, lineages involving only natural spawning were included."
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^Spawners taken as broodstock")
    )
  ),
...
### Table S`r the_tab()` <small>_AIC_</small>
...
### Table S`r the_tab()` <small>_RS & RRS_</small> {.tabset .tabset-pills}
...
### Fig. S`r the_fig()` <small>_RS Relationships_</small> {.tabset .tabset-dropdown}
...
### Fig. S`r the_fig()` <small>_RS Groups_</small>
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^Spawners taken as broodstock"),
      htmltools::tags$li("^b^Spawners with progeny taken as broodstock")
    )
  ),
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^Spawners taken as broodstock"),
      htmltools::tags$li("^b^Spawners with progeny taken as broodstock"),
      htmltools::tags$li("^c^Spawners with zero F~1~ progeny")
    )
  ),
  footnote_text = c(
    no_broodstock_footnote, no_F1_broodstock_footnote,
    "Spawners (F~0~) that produce zero progeny (F~1~) are guaranteed to produce zero grand-progeny (F~2~). If included in the hurdle model, this lack of variability at one covariate value is problematic for the zero submodel and predictions from the conditional sub-model at F~1~ = 0 will always be wrong. Thus we deemed inclusion of the F~1~ covariate required using only spawners that produced at least one F~1~ progeny; the 1-Gen Origin Type RRS analysis already established the factors driving the probability of producing zero F~1~ progeny.")
)
```
...
### Fig. S`r the_fig()` <small>_RRS_</small>

:::{.helptext}
<details>
<summary>`r shiny::icon("info-circle")` **Description**</summary>
_Relative reproductive success (RRS) ratios from hurdle model-predicted reproductive success values of reproductive success by year, origin, sex, and life stage._
_Error bars represent 95% bootstrap confidence intervals._
_Values shown here are identical to those in Table S6._
</details>
:::
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^Spawners taken as broodstock"),
      htmltools::tags$li("^b^Spawners with zero progeny")
    )
  ),
  footnote_text = c(
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      ...
      htmltools::tags$li("^a^Spawners taken as broodstock"),
      htmltools::tags$li("^b^NOR spawners"),
      htmltools::tags$li("^c^HOR spawners with missing acclimation site")
    )
  ),
  footnote_text = c(
    no_broodstock_footnote, 
...
<details>
<summary>`r shiny::icon("info-circle")` **Description**</summary>
_Relative reproductive success (RRS) ratios from hurdle model-predicted reproductive success values of reproductive success by year, sex, and acclimation site._
_Error bars represent 95% bootstrap confidence intervals._
_Values shown here are identical to those in Table S10._
</details>
:::
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^Spawners taken as broodstock"),
      htmltools::tags$li("^b^HOR spawners"),
      htmltools::tags$li("^c^Spawners with unknown origin for one or both parents")
    )
  ),
  footnote_text = c(
    no_broodstock_footnote,
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^HOR spawners")
    )
  ),
...
desc_table(
  ...
  data_rules = as.character(
    htmltools::tags$ul(
      htmltools::tags$li("^a^Spawners with progeny taken as broodstock"),
      htmltools::tags$li("^b^HOR spawners")
    )
  ),
...
### Fig. S`r the_fig()` <small>Differences in Means</small>
